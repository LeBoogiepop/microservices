...
def get_user_token():
    ...
    try:
        ...
        decoded = jwt.decode(access_token, app.config['SECRET_KEY'], claims_options=claims_options)
        # Si decoded est un objet Claims, convertir en dict
        if hasattr(decoded, 'validate'):
            # Ne pas valider l'expiration
            decoded.validate(leeway=999999999)
            payload = dict(decoded)
        else:
            payload = decoded
        exp = payload.get('exp', 0)
        # Convertir exp en timestamp si c'est un datetime
